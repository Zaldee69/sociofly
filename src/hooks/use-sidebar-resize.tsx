...
export function useSidebarResize({
  ...
  const persistWidth = React.useCallback((width: string) => {
    // Ensure we're storing a valid width value
    const validWidth = width.trim();
    if (!validWidth) return;

    console.log(`[Sidebar] Persisting width: ${validWidth}`);
    document.cookie = `${SIDEBAR_WIDTH_COOKIE_NAME}=${validWidth}; path=/; max-age=${SIDEBAR_WIDTH_COOKIE_MAX_AGE}`;
  }, []);
  ...
  React.useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (!isDragging.current && deltaX > 5) {
        ...
        // Force immediate CSS updates during dragging
        document.documentElement.style.setProperty("--drag-in-progress", "1");
      }
      ...
      if (isDragging.current) {
        ...
        if (
          ...
          // Directly update the CSS variable as well for immediate feedback
          document.documentElement.style.setProperty(
            "--sidebar-width",
            formattedWidth
          );

          // Call resize handler to update React state
          onResize(formattedWidth);

          // Store width in cookie
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      if (!isDragging.current) {
        ...
        document.documentElement.style.removeProperty("--drag-in-progress");
      }
