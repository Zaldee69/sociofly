...
function isScheduledPost(post: any): post is IScheduledPost {
  return post && typeof post.id === 'string' && typeof post.user_id === 'string';
}
...
export class CalendarService {
  ...
  async getScheduledPosts(filters?: {
    ...
    return data.map(this.mapScheduledPostToEvent);
  }
  ...
  async createScheduledPost(event: IEvent): Promise<IEvent> {
    ...
    return this.mapScheduledPostToEvent(data);
  }
  ...
  mapScheduledPostToEvent(post: IScheduledPost): IEvent {
    ...
    return {
      ...
      endDate: new Date(new Date(post.scheduled_time).getTime() + 3600000).toISOString(), // Default 1 hour duration
  ...
  private async setupSubscription(callback: (payload: RealtimePayload) => void) {
    try {
  ...
  private handleSubscriptionError(callback: (payload: RealtimePayload) => void) {
