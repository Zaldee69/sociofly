...
import { calculateAnalytics } from "./analytics/content/content-analytics";
...
export async function enhancedGenerate({
  ...
  optimizedCaption = optimizeLength(
    optimizedCaption,
    platform as keyof typeof platformConfig
  );
  optimizedCaption = optimizeForPlatform(
    optimizedCaption,
    platform as keyof typeof platformConfig,
    industry as keyof typeof industryConfig
  );
  ...
  return {
    ...
    hashtags: hashtags.slice(
      0,
      platformConfig[platform as keyof typeof platformConfig].hashtagLimit
    ),
