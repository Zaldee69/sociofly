...
function calculateEngagementScore(
  ...
  const platformConfig = lengthScores[platform as keyof typeof lengthScores] || lengthScores.instagram;
  const lengthScore = Math.max(0, 100 - Math.abs(text.length - platformConfig.optimal) / platformConfig.max * 100);
  ...
  const ctaPhrases = ['click', 'link', 'comment', 'share', 'follow', 'like', 'save'];
...
function analyzeSentiment(text: string): 'positive' | 'neutral' | 'negative' {
  ...
  const positiveWords = [
    'great', 'amazing', 'wonderful', 'excellent', 'best', 'love', 'happy', 'excited',
    'fantastic', 'brilliant', 'perfect', 'awesome', 'incredible', 'beautiful', 'stunning',
    'impressive', 'outstanding', 'superb', 'magnificent', 'delightful'
  ];
  
  const negativeWords = [
    'bad', 'terrible', 'awful', 'worst', 'hate', 'sad', 'angry', 'disappointed',
    'poor', 'horrible', 'dreadful', 'miserable', 'annoying', 'frustrating', 'upset',
    'disgusting', 'awful', 'terrible', 'horrendous', 'atrocious'
  ];
...
function calculateReadabilityDetails(text: string): EnhancedAnalytics['readabilityDetails'] {
...
function calculatePlatformSpecificScore(text: string, platform: string): number {
  let score = 0;
...
function calculateContentQuality(
  ...
  const brandAlignment = industry ? calculateBrandAlignment(text, industry) : 50;
...
function calculateToneConsistency(text: string): number {
  ...
  const formalMarkers = /(?:therefore|however|furthermore|consequently|moreover)/i;
  ...
  const toneCount = [hasFormal, hasCasual, hasPromotional].filter(Boolean).length;
...
function calculateBrandAlignment(text: string, industry: IndustryType): number {
  ...
  const keywordScore = keywords.reduce((score: number, keyword: string) => 
    score + (text.toLowerCase().includes(keyword.toLowerCase()) ? 20 : 0), 0
  );
...
function calculateUniquenessScore(text: string): number {
  ...
  const uniqueWords = Array.from(wordFrequency.entries())
...
export function calculateAnalytics(
  ...
  const baseAnalytics = {
    engagementScore: calculateEngagementScore(content.caption, content.hashtags, platform, industry),
  ...
  return {
    ...
    engagementDetails: calculateEngagementDetails(content.caption, content.hashtags, industry),
    platformOptimization: calculatePlatformOptimization(content.caption, platform),
...
// Access the results
console.log(analysis.engagementScore); // e.g., 85
console.log(analysis.sentiment); // e.g., "positive"
console.log(analysis.contentQuality.grammarScore); // e.g., 90
console.log(analysis.engagementDetails.viralPotential); // e.g., 75 