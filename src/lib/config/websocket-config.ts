...
export const getWebSocketConfig = (): WebSocketConfig => {
  ...
  if (process.env.WS_MAX_NOTIFICATIONS_PER_USER) {
    config.maxNotificationsPerUser = parseInt(process.env.WS_MAX_NOTIFICATIONS_PER_USER);
  }
  
  if (process.env.WS_CLEANUP_INTERVAL_MINUTES) {
    config.cleanupIntervalMinutes = parseInt(process.env.WS_CLEANUP_INTERVAL_MINUTES);
  }
  
  if (process.env.WS_ENABLE_REDIS_FALLBACK) {
    config.enableRedisFallback = process.env.WS_ENABLE_REDIS_FALLBACK === 'true';
  }
...
export class WebSocketPerformanceTracker {
  ...
  public recordNotificationSent(deliveryMethod: 'websocket' | 'database' | 'redis', deliveryTimeMs: number): void {
  ...
  public getEfficiencyStats(): {
    ...
    return {
      ...
      redisReductionPercentage: 100 - ((this.metrics.redisOperations / total) * 100),
    };
  }
