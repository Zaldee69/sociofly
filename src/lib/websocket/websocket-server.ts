...
const loadSocketIO = async () => {
  if (typeof window !== "undefined" || isSocketIOLoaded) {
    console.log(
      "🔍 loadSocketIO skipped - window:",
  ...
  try {
    ...
    console.log("🔍 socketIO module:", Object.keys(socketIO));
    console.log("🔍 socketIO.Server:", socketIO.Server);
    console.log("🔍 socketIO.default:", socketIO.default);

    // Try different ways to access the Server class
    SocketIOServer =
      socketIO.Server || socketIO.default?.Server || socketIO.default;
    isSocketIOLoaded = true;
    console.log("✅ Socket.IO modules loaded successfully");
    console.log("🔍 SocketIOServer type:", typeof SocketIOServer);
    console.log(
      "🔍 SocketIOServer value:",
      SocketIOServer ? "available" : "null/undefined"
    );
  } catch (error) {
    console.error("❌ Failed to load Socket.IO modules:", error);
    throw error;
  }
};

// Load Socket.IO immediately if on server-side
...
export class WebSocketServer {
  ...
  private inMemoryNotifications: Map<string, InMemoryNotification[]> =
    new Map(); // userId -> notifications
  ...
  private async sendPendingNotifications(
    ...
    try {
      ...
      const inMemoryNotifications =
        this.inMemoryNotifications.get(userId) || [];
      ...
      console.log(
        `📨 Sent ${unreadInMemory.length} in-memory notifications to user ${userId}`
      );
      ...
      if (unreadInMemory.length === 0) {
        try {
          ...
          console.log(
            `📨 Sent ${dbNotifications.notifications.length} database notifications to user ${userId}`
          );
        } catch (dbError) {
  ...
  private async markNotificationAsRead(
    ...
    try {
      ...
      if (foundInMemory) {
        ...
        console.log(
          `✅ Marked in-memory notification ${notificationId} as read for user ${userId}`
        );
      } else {
  ...
  private startInMemoryCleanup(): void {
    ...
    this.cleanupInterval = setInterval(() => {
      ...
      const totalNotifications = Array.from(
        this.inMemoryNotifications.values()
  ...
  private cleanupExpiredNotifications(): void {
    ...
    for (const [
      userId,
      notifications,
    ] of this.inMemoryNotifications.entries()) {
      const validNotifications = notifications.filter(
        (notification) =>
          now - new Date(notification.timestamp).getTime() < expiryTime
      );
    ...
    if (totalCleaned > 0) {
      console.log(
        `[WebSocket] Cleaned up ${totalCleaned} expired notifications. Memory: ${this.memoryMonitor.getMemoryUsageMB().toFixed(2)}MB`
      );
    }
  }
  ...
  private performAggressiveCleanup(): void {
    console.log(
      "[WebSocket] Performing aggressive cleanup due to high memory usage"
    );
    ...
    console.log(
      `[WebSocket] Aggressive cleanup completed. New memory usage: ${this.memoryMonitor.getMemoryUsageMB().toFixed(2)}MB`
    );
  }
  ...
  private startPerformanceMonitoring(): void {
    ...
    setInterval(() => {
      this.performanceTracker.updateConnectionCount(
        this.io.sockets.sockets.size
      );
      this.performanceTracker.updateMemoryUsage(
  ...
  public async sendNotificationToUser(
    ...
    try {
      ...
      const inMemoryNotification: InMemoryNotification = {
        ...notification,
        expiresAt: new Date(
          Date.now() + this.config.notificationExpiryHours * 60 * 60 * 1000
        ),
      };
      ...
      if (isUserOnline) {
        ...
        console.log(
          `📨 Real-time notification sent to user ${userId}: ${notification.title}`
        );

        // Track performance
        const deliveryTime = Date.now() - startTime;
        this.performanceTracker.recordNotificationSent(
          "websocket",
          deliveryTime
        );
        ...
        console.log(
          `📨 Notification stored in memory for offline user ${userId}: ${notification.title}`
        );
  ...
  public async sendNotificationToTeam(
    teamId: string,
    notification: NotificationPayload
  ): Promise<void> {
    try {
      ...
      console.log(
        `📨 Team notification sent to ${onlineUsers.length} users in team ${teamId}: ${notification.title}`
      );
    } catch (error) {
  ...
  public async broadcastSystemNotification(
    ...
    try {
      ...
      console.log(
        `📢 System notification broadcasted to ${this.connectedUsers.size} users: ${notification.title}`
      );
    } catch (error) {
  ...
  public getOnlineUsersInTeam(teamId: string): string[] {
    ...
    this.connectedUsers.forEach((connections, userId) => {
      const hasTeamConnection = connections.some(
        (conn) => conn.teamId === teamId
      );
  ...
  private async persistNotificationToDatabase(
    notification: NotificationPayload
  ): Promise<void> {
    try {
  ...
  public getInMemoryStats(): {
    ...
    return {
      ...
      averageNotificationsPerUser:
        Math.round(averageNotificationsPerUser * 100) / 100,
      memoryUsageEstimate,
      activeConnections,
  ...
  public async shutdown(persistUnreadNotifications?: boolean): Promise<void> {
    const shouldPersist =
      persistUnreadNotifications ?? this.config.enableGracefulShutdown;
    ...
    if (shouldPersist && this.config.enableDatabaseFallback) {
      ...
      if (stats.totalNotifications > 0) {
        console.log(
          `[WebSocket] Persisting ${stats.totalNotifications} in-memory notifications before shutdown...`
        );
          ...
          for (const notification of notifications) {
            if (!notification.read) {
              try {
                ...
                console.error(
                  `Failed to persist notification for user ${userId}:`,
                  error
                );
              }
            }
        ...
        console.log(
          `[WebSocket] Persisted ${persistedCount} unread notifications`
        );
      }
    }
...
// Type declaration for global WebSocket server instance
declare global {
  var __webSocketServer: WebSocketServer | null;
}

// Singleton instance with global storage for cross-context access
let webSocketServer: WebSocketServer | null = null;

// Store in global for cross-context access in Node.js
if (typeof global !== "undefined" && typeof window === "undefined") {
  if (!global.__webSocketServer) {
    global.__webSocketServer = null;
  }
}
...
export const getWebSocketServer = (): WebSocketServer | null => {
  ...
  if (webSocketServer) {
    console.log(
      "🔍 getWebSocketServer called - webSocketServer: available (local)"
    );
    return webSocketServer;
  }

  // Try global instance for cross-context access
  if (typeof global !== "undefined" && global.__webSocketServer) {
    console.log(
      "🔍 getWebSocketServer called - webSocketServer: available (global)"
    );
    webSocketServer = global.__webSocketServer;
    return webSocketServer;
  }

  console.log("🔍 getWebSocketServer called - webSocketServer: null");
  return null;
};
...
export const initializeWebSocketServer = async (
  ...
  if (typeof window !== "undefined") {
    console.log("🔍 initializeWebSocketServer skipped - running in browser");
    return null;
  }
  ...
  try {
    ...
    console.log(
      "🔍 Before loadSocketIO - SocketIOServer:",
      SocketIOServer ? "available" : "null/undefined"
    );
    console.log("🔍 Before loadSocketIO - isSocketIOLoaded:", isSocketIOLoaded);

    // Ensure Socket.IO is loaded first
    await loadSocketIO();

    console.log(
      "🔍 After loadSocketIO - SocketIOServer:",
      SocketIOServer ? "available" : "null/undefined"
    );
    console.log("🔍 After loadSocketIO - isSocketIOLoaded:", isSocketIOLoaded);
    console.log("🔍 webSocketServer exists:", webSocketServer ? "yes" : "no");

    if (!webSocketServer && SocketIOServer) {
      console.log("🔄 Creating new WebSocketServer instance...");
      webSocketServer = new WebSocketServer(server);

      // Store in global for cross-context access
      if (typeof global !== "undefined") {
        global.__webSocketServer = webSocketServer;
        console.log("🔄 WebSocket server stored in global context");
      }

      console.log("🚀 WebSocket server initialized successfully");
    } else if (!SocketIOServer) {
      console.error("❌ SocketIOServer not available after loading");
      console.error("🔍 SocketIOServer type:", typeof SocketIOServer);
      console.error("🔍 SocketIOServer value:", SocketIOServer);
      return null;
    } else if (webSocketServer) {
      console.log(
        "🔍 WebSocket server already exists, returning existing instance"
      );
    }
  } catch (error) {
